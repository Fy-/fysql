...
class TestSelect(unittest.TestCase):
    ...
    def test_fkey_where(self):
        d = Post.where(User.id==1).sql
    ...
    def test_fkey_where_limit(self):
        d = Post.where(User.id==1).limit(1).sql
        r = 'SELECT `post`.`id` AS post_id,`post`.`id_user` AS post_id_user,`post`.`title` AS post_title,`user`.`id` AS user_id,`user`.`lastname` AS user_lastname,`user`.`firstname` AS user_firstname FROM `post` INNER JOIN `user` ON `post`.`id_user`=`user`.`id` WHERE `user`.`id` = 1 LIMIT 0,1'
